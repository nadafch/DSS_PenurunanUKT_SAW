...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($update) {
		$sql = "UPDATE kriteria SET nm_kriteria='$_POST[nm_kriteria]' WHERE id_kriteria='$_GET[key]'";
	} else {
		$sql = "INSERT INTO kriteria VALUES (NULL,'$_POST[nm_kriteria]')";
		$validasi = true;
	}

	if ($validasi) {
		$q = $connection->query("SELECT id_kriteria FROM kriteria WHERE nm_kriteria LIKE '%$_POST[nm_kriteria]%'");
		if ($q->num_rows) {
	        ...
	        <div class="panel-body">
	            <form action="<?=$_SERVER['REQUEST_URI']?>" method="POST">
					<div class="form-group">
	                    ...
	                    <input type="text" name="nm_kriteria" class="form-control" <?= (!$update) ?: 'value="'.$row["nm_kriteria"].'"' ?>>
	                </div>
